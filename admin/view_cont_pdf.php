...
if(isset($_POST['delete_pdf'])){
   ...
   $delete_pdf_thumb = $conn->prepare("SELECT thumb FROM `doc` WHERE id = ? LIMIT 1");
   $delete_pdf_thumb->execute([$delete_id]);
   $fetch_thumb = $delete_pdf_thumb->fetch(PDO::FETCH_ASSOC);
   unlink('../uploaded_files/'.$fetch_thumb['thumb']);

   $delete_pdf = $conn->prepare("SELECT pdf FROM `doc` WHERE id = ? LIMIT 1");
   ...
   $delete_doc = $conn->prepare("DELETE FROM `doc` WHERE id = ?");
   $delete_doc->execute([$delete_id]);
   header('location:contents_pdf.php');
...
<section class="view-content">
   ...
   <?php
      $select_content = $conn->prepare("SELECT * FROM `doc` WHERE id = ? AND tutor_id = ?");
   ...
   ?>
   <div class="container">
      <pdf src="../uploaded_files/<?= $fetch_content['pdf']; ?>" autoplay controls poster="../uploaded_files/<?= $fetch_content['thumb']; ?>" class="pdf"></pdf>
   ...
   <?php
      ...
      echo '<p class="empty">no contents added yet! <a href="add_content.php" class="btn" style="margin-top: 1.5rem;">add pdf</a></p>';
   }
